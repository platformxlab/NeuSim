...
NeuSim is a simulator framework for modeling the performance and power of neural processing units (NPUs) running deep neural network (DNN) workloads.
...
### Quick Start
The `example_npusim.sh` script launches a set of experiments to verify the installation of NeuSim.
This script represents a typical workflow of running batch simulations with NeuSim:
- First, it invokes `run_sim.py` for different DNN models to run performance simulations. This script is the main entry point for running a batch of performance simulations. It sweeps over all possible number of chips, batch sizes, NPU versions, and parallelism configurations for the given DNN models, and dumps the per-operator performance statistics for each experiment to CSV files. This script will launch multiple Ray tasks to parallelize the simulation jobs.
...
Follow the steps below to run the example script:
...
1. Start ray server:
    ...
    Optional: To speed up the simulation, we can run the experiments on a Ray cluster with multiple machines. Please refer to [Running on a Ray Cluster](#running-on-a-ray-cluster).
...
The simulation results are dumped as CSV files under the specified output directory.
Each CSV file contains the per-operator statistics (operator definition, execution time, memory traffic, power, energy, etc.) for a single experiment.
The [`Operator`](neusim/npusim/frontend/Operator.py) class contains the descriptions for all the statistics fields for each operator. Most fields are self-explanatory.
...
- `configs/models/`: contains the model architecture parameters as well as the parallelism configurations. We currently support LLMs (Llama and DeepSeek), DLRM, DiT-XL, and GLIGEN. See [Adding New DNN Models](#adding-new-dnn-models) for more details on how to add support for new models.
...
The scripts `neusim/run_scripts/energy_operator_analysis_main.py` and `neusim/run_scripts/carbon_analysis_main.py` supports different parameters via command line arguments.
The `--help` option can be used to view all available arguments. To perform sensitivity study for power gating parameters, these two scripts support overriding the default power gating configurations via the `--power_gating_strategy` flag as follows:
...
### Writing Unit Tests
All tests covering a certain module should be placed under the `test` directory under that module.
All test files should be named with the `test_*.py` format. If a unit test requires inputs from files, the input files should be placed under the `test` directory (preferrably inside a sub-directory of `test`). See `neusim/backend/test` for examples.
