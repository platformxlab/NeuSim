...
def _fill_ga_offspring_scalar_exact(
    ...
    *,
    ...
    while offspring_index < population_size:
        ...
        for gene_index in range(num_genes):
            if rng.random() < mutation_prob:
                child1_indices.append(gene_index)
                child1_steps.append(rng.choice([-1, 1]))
            if rng.random() < mutation_prob:
                child2_indices.append(gene_index)
                child2_steps.append(rng.choice([-1, 1]))
...
def _configured_operator_candidates(
    ...
    for candidate_config in configs:
        candidate = op.model_copy()
        candidate.stats = op.stats.model_copy()
        # Keep component objects independent because energy analysis annotates
        # regulator efficiency on each selected configuration.
        candidate.dvfs_sa = candidate_config["sa"].model_copy()
        candidate.dvfs_vu = candidate_config["vu"].model_copy()
        candidate.dvfs_sram = candidate_config["sram"].model_copy()
        candidate.dvfs_hbm_mc = candidate_config["hbm_mc"].model_copy()
        candidate.dvfs_hbm_die = candidate_config["hbm_die"].model_copy()
        candidate.dvfs_hbm_io = candidate_config["hbm_io"].model_copy()
        candidate.dvfs_ici_mc = candidate_config["ici_mc"].model_copy()
        candidate.dvfs_ici_phy = candidate_config["ici_phy"].model_copy()
        yield candidate
...
def _iter_bounded_analyzed_candidate_batches(
    candidates: Iterable[Operator.Operator],
    config: ModelConfig,
    pg_config: "str | PowerGatingConfig | None",
    dvfs_config: DVFSConfig,
    ignore_vr_power_loss: bool,
    *,
    ...
    if serial:
        ...
        while batch := _next_candidate_batch(candidate_iterator, batch_size):
            ...
            analyzed = _analyze_operator_energy_batch(
                batch,
                config,
                pg_config,
                dvfs_config,
    ...
    analyze_remote = ray.remote(_analyze_operator_energy_batch)
    ...
    try:
        while pending or not exhausted:
            while len(pending) < window_size and not exhausted:
                ...
                pending.append((batch_ordinal, len(batch), future))
            ...
            if pending:
                batch_ordinal, expected_count, future = pending.popleft()
...
def _extract_indexed_pareto_front(
    ...
    ordered = sorted(indexed_ops, key=lambda item: _indexed_pareto_sort_key(item, policy))
    if policy in (DVFSPolicy.DVFS_C, DVFSPolicy.DVFS_C_ms):
    ...
    for indexed_op in ordered:
        if (
            not result
            or indexed_op[1].stats.total_energy_J
            < result[-1][1].stats.total_energy_J
        ):
            result.append(indexed_op)
    return result
...
def _write_analyzed_dump_run(
    ...
    with open(path, "wb") as output:
        for indexed_op in ordered:
            pickle.dump(
                (_indexed_pareto_sort_key(indexed_op, policy), indexed_op[1].to_csv_dict()),
                output,
...
def generate_pareto_energy_latency_points_for_op_exhaustive_search(
    ...
    if dvfs_config.policy == DVFSPolicy.IDEAL:
        ...
        all_dvfs_configs: Iterable[dict[str, ComponentDVFSConfig]] = (
            iter_all_dvfs_configs_for_op(
                op,
                dvfs_config.policy,
    ...
    candidates = _configured_operator_candidates(op, config_iterator)
    ...
    run_stats: dict = {
        **batch_stats,
        ...
        "config_enumeration": (
            "lazy_cartesian" if dvfs_config.policy == DVFSPolicy.IDEAL else "materialized"
        ),
...
def generate_pareto_energy_latency_points_for_op_greedy_search(
    ...
    while True:
        ...
        for candidate_op, trial_indices in zip(candidate_ops, candidate_indices_list, strict=False):
            delta_time = (
...
def _ms_precompute(ops, config, interval_ns, pg_config):
    ...
    if not dvfs_serial:
        ...
        analyze_remote = ray.remote(analyze_operator_energy)
    peak = ComponentDVFSConfig(
        policy=DVFSPolicy.NONE,
        voltage_V=0.7,
        frequency_GHz=1.7,
    ...
    for i, op in enumerate(ops):
        ...
        for pt in SA_POINTS:
            ...
            if dvfs_serial:
                ...
                futures.append(
                    analyze_remote.remote(
                        _op, config, pg_config, dvfs_for_eval, False, False
                    )
                )
        evaluated = futures if dvfs_serial else ray.get(futures)
