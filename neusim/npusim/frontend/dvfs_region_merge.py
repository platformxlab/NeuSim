...
def _relabel(
    ...
    if set(duration_by_label).issubset({HFC, LFC}):
        region.label = (
            HFC
            if duration_by_label.get(HFC, 0.0)
            >= duration_by_label.get(LFC, 0.0)
            else LFC
        )
        return
...
def build_initial_regions(
    ...
    for index, (label, count) in enumerate(zip(labels, counts, strict=True)):
        if (
            regions
            and regions[-1].label == label
            and regions[-1].count == count
        ):
...
def _collapse_adjacent(
    ...
    for region in regions[1:]:
        if (
            region.label == output[-1].label
            and region.count == output[-1].count
        ):
    ...
    while len(merged_regions) > 1:
        ...
        for source in order:
            ...
            left = (
                source - 1
                if source > 0
                and merged_regions[source - 1].count
                == merged_regions[source].count
                else None
            )
            right = (
                source + 1
        ...
        combined = FrequencyRegion(
            op_indices=(
                merged_regions[low].op_indices
                + merged_regions[high].op_indices
            ),
            label=HFC,
...
def build_regions_by(
    ...
    for region in regions:
        region.duration_ns = sum(
            durations[index] for index in region.op_indices
        )

    flattened = tuple(
        index for region in regions for index in region.op_indices
    )
...
def merge_region_to_operator(
    ...
    if len(distinct_counts) != 1:
        raise ValueError(
            "merge requires a uniform operator count, got "
            f"{sorted(distinct_counts)}"
        )
    ...
    for field_name, field_info in type(stats).model_fields.items():
        ...
        if field_info.annotation is int:
            values = [
                getattr(member.stats, field_name, 0)
...
def build_merged_operator_series(
    ...
    merged_ops = [
        merge_region_to_operator(ops, region, config)
        for region in regions
    ]
