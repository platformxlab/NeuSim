...
def _full_config(grids, indices):
    ...
    die_points = sorted(
        (
            point
            for point in getter.HBM_DIE_POINTS
            if abs(point.voltage_V - 1.2) < 0.02
        ),
    ...
    die = next(
        (
            ...
            if point.bandwidth_GBs >= target_bw - 1.0
        ),
        die_points[-1],
    )
    io = next(
...
def _analyze_many(
    ...
    """Analyze jobs serially for tests or through the active Ray runtime."""
    if os.environ.get("DVFS_PARETO_SERIAL") == "1":
        return [
            analyze(job, config, pg_config, dvfs_config, False, False)
            for job in jobs
        ]
    import ray

    remote = remote_analyze or ray.remote(analyze)
    return ray.get(
        [
            remote.remote(job, config, pg_config, dvfs_config, False, False)
            for job in jobs
        ]
    )
...
def _precompute(ops, config, interval_ns, pg_config):
    ...
    if os.environ.get("DVFS_PARETO_SERIAL") != "1":
        ...
        # Match the recovered implementation's one remote wrapper per
        # precompute. Creating and exporting a new Ray function for every
        # operator is needlessly expensive on long LLM traces.
        remote_analyze = ray.remote(analyze_operator_energy)
    ...
    for op_index, op in enumerate(ops):
        ...
        domain_time = {
            domain: np.zeros(grid_sizes[domain])
            for domain in _DOMAINS
        }
        subdata = {
            domain: {
        ...
        for (domain, grid_index), result in zip(
            ...
            for sub in _DOMAIN_SUBS[domain]:
                ...
                efficiency = (
                    component.voltage_conversion_power_efficiency_percent
                )
                values = subdata[domain][sub]
                values["raw_dynamic"][grid_index] = (
                    getattr(stats, f"dynamic_energy_{sub}_J")
                    * efficiency
                    / 100.0
                )
                ...
                values["voltage"][grid_index] = float(
                    component.voltage_V
                )
                values["scaling_ns"] = (
                    component.voltage_regulator_scaling_time_ns
                )

        phy_component = peak_result.dvfs_ici_phy
        phy_efficiency = (
            phy_component.voltage_conversion_power_efficiency_percent
        )
        ici_phy = {
            "raw_dynamic": (
                peak_stats.dynamic_energy_ici_phy_J
                * phy_efficiency
                / 100.0
            ),
            "static_power": (
                peak_stats.static_energy_ici_phy_J
                * phy_efficiency
                / 100.0
                / peak_time
                if peak_time > 0.0
            ...
            "scaling_ns": (
                phy_component.voltage_regulator_scaling_time_ns
            ),
        }
        other_power = (
            (
                peak_stats.dynamic_energy_other_J
                + peak_stats.static_energy_other_J
            )
            / peak_time
            if peak_time > 0.0
            else 0.0
        ...
        pre_ops.append(
            {
                ...
                "flops_util": float(
                    min(1.0, max(0.0, op.stats.flops_util))
                ),
    ...
    peak_plan = np.array(
        [[grid_sizes[domain] - 1 for domain in _DOMAINS]]
        * len(regions),
        dtype=np.int32,
    )
...
def _run_ga(
    ...
    *,
    ...
    if (
        seed_population is not None
        and seed_population.shape == population.shape
    ):
    ...
    def score(values):
        ...
        valid = (
            ...
            & (
                total_time
                <= allowed_time + max(1e-6, abs(allowed_time) * 1e-12)
            )
        )
        with np.errstate(divide="ignore", invalid="ignore"):
            objective = (
                baseline_time / total_time
                * (baseline_energy / total_energy) ** 3
            )
    ...
    for _ in range(max_generations):
        ...
        if chromosome_length > 1:
            do_crossover = (
                random_state.random(offspring_count) < crossover_prob
            )
        ...
        mutate = (
            random_state.random((offspring_count, chromosome_length))
            < mutation_prob
        )
...
def _measure_schedule(
    ...
    total_time = sum(
        float(op.stats.execution_time_ns) * int(op.stats.count)
        for op in evaluated
    )
    total_energy = sum(
        float(op.stats.total_energy_J) * int(op.stats.count)
        for op in evaluated
    )
    return total_time, total_energy
...
def configure_enpu_ms_all_budgets(
    ...
    for budget in sorted_budgets:
        ...
        if not feasible and exact_fallback_candidates:
            ...
            model_feasible = np.flatnonzero(
                population_times
                <= model_allowed
                + max(1e-6, abs(model_allowed) * 1e-12)
            )
            ranked = model_feasible[
                np.argsort(population_energies[model_feasible])
            ]
            ...
            for population_index in ranked:
                ...
                exact_measurement_seconds += (
                    time.perf_counter() - measurement_start
                )
                exact_candidate_counts[budget] += 1
                if (
                    retry_time
                    <= allowed + max(1e-5, abs(allowed) * 1e-12)
                    and (
                        retry_best is None
                        or retry_energy < retry_best[3]
                    )
                ):
                    retry_best = (
                        retry_plan.copy(),
        ...
        results[budget] = [
            op.model_copy(deep=True)
            for op in incumbent_schedule
        ]
    ...
    timings = {
        ...
        "movement_from_fastest_pct": {
            budget: 100.0
            * (selected_times[budget] - peak_time)
            / raw_time
            for budget in sorted_budgets
        },
...
def configure_enpu_ms_with_regions(
    ...
    mode = (
        getattr(dvfs_config, "custom_compute_domain_mode", "dom5")
        or "dom5"
    ).lower()
    if mode != "dom5":
        raise ValueError(
