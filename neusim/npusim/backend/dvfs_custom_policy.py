...
def slowdown_freq(
    ratio: float, base_freq_GHz: float, min_freq_GHz: float = 0.05
) -> float:
    """
...
def get_dvfs_policy_custom(
    ...
    """
    assert dvfs_cfg.policy in (
        DVFSPolicy.CUSTOM,
        DVFSPolicy.CUSTOM_ALL,
        DVFSPolicy.CUSTOM_ALL_ms,
    ), f"dvfs_mode must be CUSTOM, CUSTOM_ALL, or CUSTOM_ALL_ms, got {dvfs_cfg.policy}"
    ...
    def snap_vf_up_from_points(
        points: Sequence[VfPoint], target_f_ghz: float
    ) -> tuple[float, float]:
        """
    ...
    def is_elementwise(o: Operator.Operator) -> bool:
        ...
        return (
            o.opcode_type == OpcodeType.ELEMENTWISE
            or o.opcode_type == OpcodeType.EMBEDDING_BAG
        )
    ...
    def apply_flash_attention_caps(
        ...
        if o.stats.bounded_by == "Memory":
            ...
            if best_gap == float("inf"):
                best_sa_f = f_sa if (0.0 < f_sa <= base_freq_ghz) else base_freq_ghz
                best_gap = abs(scaled_time(sa_time, best_sa_f) + vu_softmax_time - et)
                for cand_f in sa_candidates:
                    t_sa = scaled_time(sa_time, cand_f)
                    total = t_sa + vu_softmax_time
                    gap = abs(total - et)
            ...
            for cand_f in vu_candidates:
                ...
                if feasible:
                    if (
                        (not feasible_found)
                        or gap < best_gap
                        or (gap == best_gap and cand_f < best_vu_f)
                    ):
                        feasible_found = True
                        best_gap = gap
                        best_vu_f = cand_f
    ...
    couple_compute_domains(
        plan, getattr(dvfs_cfg, "custom_compute_domain_mode", "dom5")
    )
