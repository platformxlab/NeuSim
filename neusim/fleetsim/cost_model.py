...
def pipeline_batch_monetary_cost_dollars(
    ...
    return pipeline_batch_monetary_cost_from_stage_time_dollars(
        stage_time_ns, config
    )


def pipeline_batch_monetary_cost_from_stage_time_dollars(
    stage_time_ns: int, config: LLMConfig | DeepSeekConfig
) -> float:
    """Return the same steady-state cost from a cached stage-time summary."""
    return (
        stage_time_ns
        / 1e9
