...
def test_batch_padding_fields_and_immutable_cached_summary(monkeypatch) -> None:
    captured = []
    shared_result = backend.InferenceBatchMetrics(
        prefill=backend.PhaseMetrics(stage_time_ns=11, energy_per_chip_J=1.5),
        decode=backend.PhaseMetrics(stage_time_ns=7, energy_per_chip_J=0.5),
    )
    ...
    monkeypatch.setattr(backend, "run_llm_inference_request_seqlen_cached", fake_cached)
    config = LLMConfig(
        input_seqlen=11,
        output_seqlen=7,
        global_batch_size=9,
    ...
    frozen = captured[-1]
    helper_frozen = backend.make_frozen_config_for_batch(
        config,
        requests,
        input_padding_factors=[8, 16],
        input_padding_steps=[64],
    ...
    with pytest.raises(FrozenInstanceError):
        first.prefill.stage_time_ns = 99
    second = backend.run_inference_request_batch(config, requests)
    assert first is shared_result
    assert second is shared_result
...
def test_decode_counts_are_normalized_and_must_be_divisible(monkeypatch) -> None:
    valid_prefill = [
        SimpleNamespace(
            stats=SimpleNamespace(count=1, execution_time_ns=5, total_energy_J=2.0)
        )
    ]
    valid_decode = [
        SimpleNamespace(
            stats=SimpleNamespace(count=8, execution_time_ns=3, total_energy_J=0.25)
        ),
        SimpleNamespace(
            stats=SimpleNamespace(count=16, execution_time_ns=4, total_energy_J=0.5)
        ),
    ]
    ...
    metrics = backend.run_llm_inference_request_seqlen(frozen)
    assert [op.stats.count for op in valid_decode] == [1, 2]
    assert metrics.prefill.stage_time_ns == 5
    assert metrics.prefill.energy_per_chip_J == 2.0
    assert metrics.decode.stage_time_ns == 11
    assert metrics.decode.energy_per_chip_J == 1.25
...
def test_real_neusim_backend_returns_analyzed_operators(capsys) -> None:
    ...
    metrics = backend.run_inference_request_batch(
        config, [LLMRequest(input_seqlen=32, output_seqlen=4)]
    )

    for phase in (metrics.prefill, metrics.decode):
        assert math.isfinite(phase.stage_time_ns)
        assert phase.stage_time_ns > 0
        assert math.isfinite(phase.energy_per_chip_J)
        assert phase.energy_per_chip_J > 0
