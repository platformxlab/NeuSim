...
def test_prefill_endpoint_uses_pipeline_latency_but_one_stage_cost(
    ...
    op = SimpleNamespace(
        stats=SimpleNamespace(execution_time_ns=10, total_energy_J=2.0, count=3)
    )
    monkeypatch.setattr(
...
def test_decode_endpoint_uses_pipeline_latency_but_one_stage_cost(
    ...
    config, pod_config, simulator = _pipeline_test_setup()
    op = SimpleNamespace(
        stats=SimpleNamespace(execution_time_ns=7, total_energy_J=2.0, count=2)
    )
    monkeypatch.setattr(
        endpoint.sim_backend,
        "run_inference_request_batch",
    ...
    for request in requests:
        ...
        assert request.decode_cost_per_token_dollars != pytest.approx(
            [expected_token_cost * pod_config.pipeline_parallelism_degree]
            * num_iterations
        )
