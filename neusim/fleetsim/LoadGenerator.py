...
class LLMRequest:
    ...
    def __init__(
        self, input_seqlen: int, output_seqlen: int, timestamp: int | None = None
    ):
        if input_seqlen < 1:
            raise ValueError(f"input_seqlen must be positive, got {input_seqlen}")
        if output_seqlen < 2:
            raise ValueError(f"output_seqlen must be at least 2, got {output_seqlen}")
        ...
        self.decode_energy_per_token_J: list[float] = []
        """
        Total energy for each token (i.e., each decode iteration) consumed by decode in joules.
        """

        self.prefill_cost_dollars: float = 0.0
        """Total cost of prefill in dollars."""
        self.decode_cost_per_token_dollars: list[float] = []
        """
        Total cost for each token (i.e., each decode iteration) consumed by decode in dollars.
        """

        self.config_prefill_input_seqlen: int = -1
        """Input seq len of the vPod config used for this request."""
        self.config_decode_input_seqlens: list[int] = []
        """Input seq len of the vPod config used for each decode iteration of this request."""
        self.config_decode_output_seqlens: list[int] = []
        """Output seq len of the vPod config used for each decode iteration of this request."""
        self.config_prefill_batch_size: int = -1
        """Batch size used for prefill for this request."""
        self.config_decode_batch_sizes: list[int] = []
        """Batch size used for decode iterations for this request."""
        self.config_prefill_npu_type: str = ""
        """Prefill NPU type of the vPod config used for this request."""
        self.config_decode_npu_types: list[str] = []
        """Decode NPU types of the vPod config used for each decode iteration of this request."""
        self.config_prefill_num_chips: int = -1
        """Number of chips used for prefill for this request."""
        self.config_decode_num_chips: list[int] = []
        """Number of chips used for each decode iteration of this request."""
        self.config_prefill_pcfg: str = ""
        """Prefill pod parallelism config (e.g., dpX-tpX-ppX-epX)."""
        self.config_decode_pcfgs: list[str] = []
        """Pod parallelism config (e.g., dpX-tpX-ppX-epX) for each decode iteration of this request."""
...
class LoadGenerator:
    ...
    def load_requests_from_trace(self) -> list[LLMRequest]:
        with open(self.config.trace_file_path, newline="", encoding="utf-8") as stream:
        ...
        if burst_columns <= columns:
            reqs = self._load_requests_from_trace_BurstGPT()
        elif azure_columns <= columns:
            reqs = self._load_requests_from_trace_Azure()
        else:
            raise ValueError(
                f"Unsupported trace schema in {self.config.trace_file_path!r}; "
                f"columns were {sorted(columns)!r}"
            )
        logging.info(
        ...
        if self.config.request_rate > 1.0:
            ...
            # re-assign sequential ids (copies otherwise share the source request's id)
